/**
 * Operator — Device List (US11) + Active/Deactivated toggle (A1) + Search (A2).
 */
    ...
    <div>
      ...
      <div className="toolbar card" style={{ marginBottom: 'var(--space-4)', flexWrap: 'wrap', gap: 'var(--space-3)' }}>
        ...
        <div className="field" style={{ marginBottom: 0, flex: 1, maxWidth: 360 }}>
          <label htmlFor="device-search">Search serial / IP</label>
          <input id="device-search" placeholder="e.g. RT123 or 192.168.1"
      ...
      {loading && devices.length === 0 ? <LoadingSpinner label="Loading devices…" /> : (
        <>
          <DeviceTable
            devices={devices}
            onEdit={stateFilter === 'ACTIVATED'
              ? (d) => navigate('/operator/edit-device', { state: d }) : undefined}
